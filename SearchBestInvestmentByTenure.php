<?php
 ...
 $_SESSION['TypeOfPurchase']="Investment";
 $_SESSION['TypeOfInvestment']="Fixed Deposit";
?>
...
<body id="top-header">
    ...
    <section id="about" class="section">
        ...
        <?php
            ...
            $query="SELECT * FROM `fd_schemes` WHERE Min_Tenure<='$tenure' ORDER BY Min_Tenure DESC";
            $result=$con->query($query);
            if($result-> num_rows> 0){
                while($row=$result->fetch_assoc()){
                    $aid=$row["SId"];
                    echo "<tr><td>".$row["SId"]."</td><td>".$row["Min_Amount"]."</td><td>".$row["Interest"]."</td><td>".$row["Min_Tenure"]."</td><td>".$row["Bank"]."</td><td><buttons type='button' name='apply[$aid]' class='btn btn-primary btn-block rounded-pill shadow-sm' onclick='window.location.href=\"Payment.php?id=$aid\"'>Apply Now</buttons></td></tr>";
                }
                echo "</tbody></table>";
            }
            else
